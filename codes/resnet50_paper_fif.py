...
# right after loading or creating the model
print("First layer weight checksum:", model.conv1.weight.detach().abs().sum().item())
...
# Building label list from the data (works for any #classes)
labels = np.unique(np.concatenate([y_true, y_pred]))

print(classification_report(y_true, y_pred, digits=4, zero_division=0))

# Averages (scalars → safe for :.4f)

for avg in ("macro", "weighted", "micro"):
    p, r, f1, _ = precision_recall_fscore_support(
        y_true, y_pred, labels=labels, average=avg, zero_division=0
    )
    print(f"{avg:8s} -> P={p:.4f}, R={r:.4f}, F1={f1:.4f}")
print("Accuracy:", accuracy_score(y_true, y_pred))

# 5) Per-class rows (arrays)
p, r, f1, support = precision_recall_fscore_support(
    y_true, y_pred, labels=labels, average=None, zero_division=0
)
for name, pi, ri, fi, si in zip(labels, p, r, f1, support):
    print(f"{name}: P={pi:.4f}, R={ri:.4f}, F1={fi:.4f} (n={si})")

# 6) Confusion matrix (and normalized)
cm = confusion_matrix(y_true, y_pred, labels=labels)
print("Confusion matrix:\n", cm)
cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)
print("Row-normalized CM:\n", np.round(cm_norm, 4))
