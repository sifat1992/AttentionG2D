...
train_loader = DataLoader(train_dataset, batch_size=BATCH, drop_last=True, shuffle=True)
...
## Utilizing scheduler to get a stable output
...
val_losses = []
best_accuracy = 0.0
best_model_path = "C:/Combined/Work/Gauss-2D/saved_finals/best_model_cnn.pth"  
...
for epoch in range(EPOCH):
    ...
    # ---- LR scheduler: step on VALIDATION loss only ----
    scheduler.step(val_loss)
    current_lr = optimizer.param_groups[0]['lr']

    print(f"Epoch [{epoch+1}/{EPOCH}] | Val Loss: {val_loss:.4f} | Val Acc: {val_accuracy:.2f}% | LR: {current_lr:.6f}")
